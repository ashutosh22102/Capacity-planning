from flask import Flask, render_template, request, send_file, session, redirect, url_for
import pandas as pd
import numpy as np
from sklearn.preprocessing import MinMaxScaler
from tensorflow.keras.models import Sequential
from tensorflow.keras.layers import LSTM, Dense
from tensorflow.keras.callbacks import EarlyStopping
import plotly.graph_objects as go
import io
import datetime

app = Flask(__name__)
app.secret_key = 'your_secure_secret_key'  # Replace with a secure secret key

def create_sequences(data, sequence_length):
    X, y = [], []
    for i in range(len(data) - sequence_length):
        X.append(data[i:i + sequence_length])
        y.append(data[i + sequence_length])
    return np.array(X), np.array(y)

@app.route('/', methods=['GET', 'POST'])
def index():
    plot = None
    future_df = None
    building_ids = session.get('building_ids', [])
    current_step = session.get('current_step', 1)

    if request.method == 'POST':
        if 'file' in request.files:
            # Step 1: Upload Dataset
            file = request.files.get('file')
            if file:
                try:
                    data = pd.read_csv(file)
                    if 'Building_ID' not in data.columns:
                        session['error'] = "Uploaded CSV does not contain 'Building_ID' column."
                        return redirect(url_for('index'))
                    building_ids = data['Building_ID'].unique().tolist()
                    session['building_ids'] = building_ids
                    session['data'] = data.to_json(date_format='iso', orient='split')
                    session['current_step'] = 2
                    return redirect(url_for('index'))
                except Exception as e:
                    session['error'] = f"Error processing the file: {e}"
                    return redirect(url_for('index'))
        
        elif 'building_id' in request.form:
            # Step 2: Select Building ID
            selected_building_id = request.form.get('building_id')
            if not selected_building_id:
                session['error'] = "Please select a Building ID."
                return redirect(url_for('index'))
            session['selected_building_id'] = selected_building_id
            session['current_step'] = 3
            return redirect(url_for('index'))
        
        elif 'start_date' in request.form and 'end_date' in request.form:
            # Step 3: Select Date Range
            start_date = request.form.get('start_date')
            end_date = request.form.get('end_date')
            if not start_date or not end_date:
                session['error'] = "Please select both start and end dates."
                return redirect(url_for('index'))
            session['start_date'] = start_date
            session['end_date'] = end_date
            session['current_step'] = 4
            return redirect(url_for('index'))
        
        elif 'future_days' in request.form:
            # Step 4: Predict Future Usage
            future_days = int(request.form.get('future_days', 180))
            session['future_days'] = future_days
            # Proceed to run prediction
            if 'data' in session and 'selected_building_id' in session:
                try:
                    data = pd.read_json(session['data'], orient='split')
                    selected_building_id = session['selected_building_id']
                    start_date = session['start_date']
                    end_date = session['end_date']
                    future_days = session['future_days']

                    building_data = data[data['Building_ID'] == selected_building_id]
                    used_ports_cols = [col for col in building_data.columns if 'Used Ports' in col]
                    time_data = building_data[['Building_ID'] + used_ports_cols].melt(
                        id_vars=['Building_ID'],
                        var_name='Date',
                        value_name='Used Ports'
                    )
                    time_data['Date'] = time_data['Date'].str.split('_').str[0]
                    time_data['Date'] = pd.to_datetime(time_data['Date'], errors='coerce')
                    time_data = time_data.dropna(subset=['Date']).sort_values('Date')

                    filtered_data = time_data[(time_data['Date'] >= pd.Timestamp(start_date)) & (time_data['Date'] <= pd.Timestamp(end_date))]

                    scaler = MinMaxScaler(feature_range=(0, 1))
                    scaled_data = scaler.fit_transform(time_data[['Used Ports']])

                    sequence_length = 30
                    X, y = create_sequences(scaled_data, sequence_length)
                    X = X.reshape(X.shape[0], X.shape[1], 1)

                    model = Sequential([
                        LSTM(units=50, return_sequences=False, input_shape=(X.shape[1], 1)),
                        Dense(units=1)
                    ])
                    model.compile(optimizer='adam', loss='mean_squared_error')
                    early_stopping = EarlyStopping(monitor='loss', patience=10)
                    model.fit(X, y, epochs=10, batch_size=32, callbacks=[early_stopping], verbose=0)

                    historical_predictions = model.predict(X)
                    historical_predictions = scaler.inverse_transform(historical_predictions).flatten()
                    historical_predictions = np.round(historical_predictions).astype(int)

                    # Predict future data
                    predicted_used_ports = []
                    last_30_days = scaled_data[-sequence_length:].reshape(1, sequence_length, 1)

                    for _ in range(future_days):
                        next_pred = model.predict(last_30_days)
                        predicted_used_ports.append(next_pred[0, 0])
                        last_30_days = np.append(last_30_days[:, 1:, :], next_pred.reshape(1, 1, 1), axis=1)

                    predicted_used_ports = scaler.inverse_transform(np.array(predicted_used_ports).reshape(-1, 1)).flatten()
                    predicted_used_ports = np.round(predicted_used_ports).astype(int)

                    # Future predictions dataframe
                    last_date = time_data['Date'].iloc[-1]
                    future_dates = pd.date_range(last_date, periods=future_days + 1, freq='D')[1:]
                    future_df = pd.DataFrame({'Date': future_dates, 'Forecast': predicted_used_ports})

                    # Combine historical and forecast data for interactive plot
                    historical_df = time_data[['Date', 'Used Ports']].copy()
                    historical_df['Predicted'] = pd.Series(historical_predictions, index=historical_df.index[-len(historical_predictions):])
                    combined_df = pd.concat([historical_df, future_df.set_index('Date')], axis=0)

                    # Plot interactive graph
                    fig = go.Figure()

                    # Add actual data
                    fig.add_trace(go.Scatter(x=combined_df['Date'], y=combined_df['Used Ports'],
                                             mode='lines', name='Actual Data', line=dict(color='blue')))

                    # Add predicted data
                    fig.add_trace(go.Scatter(x=combined_df['Date'], y=combined_df['Predicted'],
                                             mode='lines', name='Predicted Data', line=dict(color='green')))

                    # Add forecast data
                    fig.add_trace(go.Scatter(x=future_df['Date'], y=future_df['Forecast'],
                                             mode='lines', name='Forecast Data', line=dict(color='red')))

                    # Customize layout
                    fig.update_layout(
                        title=f"Used Ports Analysis for {selected_building_id}",
                        xaxis_title="Date",
                        yaxis_title="Used Ports",
                        legend_title="Legend",
                        template="plotly_white"
                    )
                    plot = fig.to_html(full_html=False)

                    # Store future_df in session for download
                    session['future_csv'] = future_df.to_csv(index=False)

                    # Reset current_step to display the results
                    session['current_step'] = 5

                except Exception as e:
                    session['error'] = f"Error during prediction: {e}"
                    return redirect(url_for('index'))

    # Retrieve error message if any
    error_message = session.pop('error', None)

    return render_template('index.html', 
                           plot=plot, 
                           building_ids=building_ids, 
                           future_df=future_df,
                           current_step=current_step,
                           error=error_message)

@app.route('/download', methods=['POST'])
def download():
    csv = session.get('future_csv')
    if not csv:
        session['error'] = "No prediction data available for download."
        return redirect(url_for('index'))
    return send_file(
        io.StringIO(csv),
        mimetype='text/csv',
        attachment_filename='future_predictions.csv',
        as_attachment=True
    )

if __name__ == '__main__':
    app.run(debug=True)